...
func (agent *Agent) Dial(remote, local *core.URL) (err error) {
	...
	control := &message.Control{
		...
		Local:       agent.LocalURL.String(),
		Remote:      agent.RemoteURL.String(),
		Source:      agent.ID,
...
func (agent *Agent) Serve(control *message.Control) error {
	...
	for !agent.Closed {
		...
		if err != nil {
			agent.Log("bridge.failed", logs.ErrorLevel, "%v", err)
			continue
		}
...
func (agent *Agent) handlerControl(control *message.Control) error {
	if control.InboundSide == "" {
		return nil // connect-only
	}
	isAccept := agent.isAccept(control)

	// Determine inbound/outbound URLs from InboundSide
	var inboundURL, outboundURL *core.URL
	if control.InboundSide == core.SideLocal {
		inboundURL = control.LocalURL()
		outboundURL = control.RemoteURL()
	} else {
		inboundURL = control.RemoteURL()
		outboundURL = control.LocalURL()
	}

	// Am I on the inbound side?
	amInbound := (control.InboundSide == core.SideLocal && !isAccept) ||
		(control.InboundSide == core.SideRemote && isAccept)

	var err error
	if amInbound {
		err = agent.handlerInbound(inboundURL, outboundURL, control)
	} else {
		err = agent.handlerOutbound(outboundURL, inboundURL, control)
	}
...
func (agent *Agent) handleMessage() error {
	...
	for {
		...
		case <-keepaliveTicker.C:
			...
			go func(n string) {
				if err := agent.Send(&message.Ping{Ping: n}); err != nil {
		...
		case *message.Ping:
			...
			// Echo the ping's nonce so the remote CLIENT can verify freshness.
			if err := agent.Send(&message.Pong{Pong: m.Ping}); err != nil {
				return err
			}
...
func (agent *Agent) Close(err error) {
	...
	agent.canceler()
	// Close listener FIRST so the memory channel is removed from globalBridge
	// before yamux/connHub are torn down. This prevents MemoryDial from racing
	// into a half-dead socks proxy (channel exists but yamux is closed).
...
func (agent *Agent) ConnHubStats() []ConnHubConnStat {
	if agent.parent != nil || agent.connHub == nil {
		return nil
	}
	return agent.connHub.Stats()
}

func (agent *Agent) ConnHubSnapshot() cio.ConnStatsSnapshot {
	if agent.parent != nil || agent.connHub == nil {
		return cio.ConnStatsSnapshot{}
	}
	return agent.connHub.AggregateTrafficSnapshot()
}

// BridgeCount returns the number of active bridges.
func (agent *Agent) BridgeCount() int64 {
	return atomic.LoadInt64(&agent.connCount)
}

// BridgeInfo holds exported bridge metadata for external consumers.
...
// ListBridges returns metadata for all bridges on this agent.
func (agent *Agent) ListBridges() []BridgeInfo {
	var bridges []BridgeInfo
	agent.bridgeMap.Range(func(key, value interface{}) bool {
		b := value.(*Bridge)
		info := BridgeInfo{
			ID:          b.id,
			Source:      b.source,
			Destination: b.destination,
			Active:      !b.closed.Load(),
		}
		if b.stats != nil {
			snap := b.stats.Snapshot()
			info.BytesIn = snap.BytesIn
			info.BytesOut = snap.BytesOut
			info.RateInBps = snap.RateInBps
			info.RateOutBps = snap.RateOutBps
		}
		// Capture connection addresses
		// Try remote conn first (client-side bridges), fall back to stream addr
		conn := b.remote
		if conn == nil {
			conn = b.stream
		}
		if conn != nil {
			if la := conn.LocalAddr(); la != nil {
				info.LocalAddr = la.String()
			}
			if ra := conn.RemoteAddr(); ra != nil {
				info.RemoteAddr = ra.String()
			}
		}
...
func (agent *Agent) TrafficSnapshot() cio.ConnStatsSnapshot {
	// Forked child agents share the parent's physical conn set. Keep their own
	// agent-level transport stats empty so totals do not double-count shared I/O.
	if agent.parent != nil {
		return cio.ConnStatsSnapshot{StartedAt: agent.startTime}
	}

	if agent.connHub != nil {
		snap := agent.connHub.AggregateTrafficSnapshot()
		if snap.StartedAt.IsZero() {
			snap.StartedAt = agent.startTime
		}
		return snap
	}

	if stats := cio.StatsOf(agent.Conn); stats != nil {
		snap := stats.Snapshot()
		if snap.StartedAt.IsZero() {
			snap.StartedAt = agent.startTime
		}
...
// TrafficStats preserves the legacy API while sourcing values from the live
// transport snapshot.
func (agent *Agent) TrafficStats() (bytesIn, bytesOut int64, start time.Time, lastActive time.Time) {
	snap := agent.TrafficSnapshot()
	return snap.BytesIn, snap.BytesOut, snap.StartedAt, snap.LastActive
}
...
func (agent *Agent) handleBridgeOpen(m *message.BridgeOpen) {
	...
	if v, ok := agent.pendingStreams.LoadAndDelete(m.ID); ok {
		bridge.attachStream(v.(net.Conn))
		bridge.Handler(agent)
