...
import (
	...
	"github.com/chainreactors/rem/harness/serve"
...
func TestRelayBuildsDomainRelayRequest(t *testing.T) {
	...
	bridge := serve.NewReadWriteCloser(serve.SOCKS5SuccessReply())
...
func TestRelayRejectsInvalidPassword(t *testing.T) {
	...
	_, err = plug.Relay(serverConn, serve.NewReadWriteCloser(nil))
...
func TestRelayReturnsBridgeWriteError(t *testing.T) {
	...
	bridge := serve.NewReadWriteCloser(nil)
...
func TestRelayReturnsReplyReadError(t *testing.T) {
	...
	bridge := serve.NewReadWriteCloser(serve.SOCKS5Reply(0x05))
	_, err = plug.Relay(serverConn, bridge)
