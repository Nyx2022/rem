...
type Console struct {
	...
	*core.URLs
	...
	relayStarted bool // relay listeners started (only once)
}
...
func prepareClientConsoleURL(address *core.URL, agentID string) *core.URL {
	if address == nil {
		return nil
	}
	prepared := address.Copy()
	if agentID == "" {
		return prepared
	}
	if prepared.Tunnel == core.SimplexTunnel {
		switch prepared.RawScheme {
		case "oss":
			if prepared.GetQuery("client_id") == "" {
				prepared.SetQuery("client_id", agentID)
			}
		}
	}
	return prepared
	...
	if c.Config.IsServerMode {
		...
		for {
			...
			if err == nil && c.Config.IsRelayMode && !c.relayStarted {
				c.relayStarted = true
				StartRelayListeners(c.ConsoleURL, c.Config.RelayListenURLs, age.ID, c.Config)
			}
			if err != nil {
...
func (c *Console) Dial(address *core.URL) (*agent.Agent, error) {
	...
	dialURL := prepareClientConsoleURL(address, a.ID)
	conn, err := c.tunnel.Dial(dialURL.String())
	if err != nil {
		return nil, err
	}
	...
	for _, extra := range c.Config.ExtraServes {
		...
		ctrl := &message.Control{
			InboundSide: c.Config.InboundSide,
			Source:      alias,
			Destination: a.ID,
			Local:       extra.LocalURL.String(),
			Remote:      extra.RemoteURL.String(),
		}
...
func (c *Console) DialDirectionalPair(upURL, downURL *core.URL) (*agent.Agent, error) {
	...
	upDialURL := prepareClientConsoleURL(upURL, a.ID)
	downDialURL := prepareClientConsoleURL(downURL, a.ID)

	// Dial up channel (uses Console's own tunnel, which matches upURL's protocol)
	upConn, err := c.tunnel.Dial(upDialURL.String())
	if err != nil {
	...
	downConn, err := downTun.Dial(downDialURL.String())
	if err != nil {
		upConn.Close()
...
func (c *Console) Fork(raw string, args []string) (*agent.Agent, error) {
	...
	forked, err := a.Fork(&message.Control{
		InboundSide: r.InboundSide,
		Source:      r.Alias,
		Destination: r.Alias,
		Local:       r.URLs.LocalURL.String(),
...
func (c *Console) finishAccept(conn net.Conn, login *message.Login) (*agent.Agent, error) {
	...
	server, err := agent.NewAgent(&agent.Config{
		Alias:       login.Agent,
		Type:        core.SERVER,
		InboundSide: control.InboundSide,
		...
		URLs: &core.URLs{
			...
			LocalURL:   control.LocalURL(),
			RemoteURL:  control.RemoteURL(),
		},
	...
	viaInfo := ""
	if server.Via != "" {
		viaInfo = fmt.Sprintf(", via: %s", server.Via)
	}
