...
const (
	DefaultOSSInterval = 2000              // 2000 milliseconds (cloud storage API, match cloud storage)
...
// Timeout is per-request HTTP timeout only; logical conn reliability belongs
// to the upper ARQ layer.
...
func (config *OSSConfig) GetARQConfig() arq.ARQConfig {
	return config.ARQ
}

func normalizeOSSConfig(config *OSSConfig) {
	if config.Interval <= 0 {
		config.Interval = time.Duration(DefaultOSSInterval) * time.Millisecond
	}
	if config.MaxBodySize <= 0 {
		config.MaxBodySize = MaxOSSMessageSize
	}
	if config.Timeout <= 0 {
...
func NewOSSServer(network, address string) (Simplex, error) {
	...
	httpClient := createHTTPClient(proxyURL, config.Timeout)
