...
const channelBufferBackpressurePollInterval = time.Millisecond
...
// PutWait applies bounded backpressure by retrying until the packet is queued,
// the buffer is closed, or ctx is cancelled.
func (ch *ChannelBuffer) PutWait(ctx context.Context, data []byte) error {
	if len(data) == 0 {
		return nil
	}
	for {
		...
		time.Sleep(channelBufferBackpressurePollInterval)
	}
}
